...
// TestEngine_UpheldDisputedEntryExcludedFromBuyFlow is an integration test that
// verifies the Layer 0 correctness gate in findCandidates: an entry with an
// operator-upheld dispute must NOT appear in match results for a buy order.
//
// Flow:
//  1. Seed inventory: seller puts an entry → operator accepts it.
//  2. Operator upholds a dispute against that entry (settle:dispute + verdict:accepted).
//  3. Buyer sends a buy order whose task description matches the entry.
//  4. Engine emits a match response with no results (or omits the disputed entry).
func TestEngine_UpheldDisputedEntryExcludedFromBuyFlow(t *testing.T) {
	t.Parallel()
	h := newTestHarness(t)
	eng := h.newEngine()

	// Step 1: Seller puts an entry and operator accepts it.
	entryDesc := "Go HTTP handler unit test generator table-driven JSON validation"
	putMsg := h.sendMessage(h.seller,
		putPayload(entryDesc, "sha256:"+fmt.Sprintf("%064x", 300), "code", 10000, 20000),
		[]string{exchange.TagPut, "exchange:content-type:code", "exchange:domain:go"},
		nil,
	)
	msgs, _ := h.st.ListMessages(h.cfID, 0)
	...
	// Step 2: Operator upholds a dispute against the entry (verdict:accepted).
	disputeP, _ := json.Marshal(map[string]any{
		"phase":        "dispute",
		"entry_id":     entryID,
		"dispute_type": "content_mismatch",
		"reason":       "cached inference did not match the task description",
	})
	h.sendMessage(h.operator, disputeP,
		[]string{
			exchange.TagSettle,
	...
	if !eng.State().HasUpheldDispute(entryID) {
		t.Fatal("precondition: HasUpheldDispute must be true after operator upholds dispute")
	}

	// Step 3: Buyer sends a buy order whose task closely matches the disputed entry.
	h.sendMessage(h.buyer,
		buyPayload("Write table-driven unit tests for a Go HTTP handler accepting JSON POST", 50000),
		[]string{exchange.TagBuy},
		nil,
	)

	// Step 4: Run the engine until it emits a match message.
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	if len(matchMsgs) <= len(preMsgs) {
		// The engine emitted no match at all — that also satisfies the gate
		// (no disputed entry in results). The test passes by the absence of results.
		return
	}

	// Parse the match payload and verify the disputed entry is absent.
	matchMsg := matchMsgs[len(matchMsgs)-1]
	var mp struct {
		Results []struct {
			EntryID string `json:"entry_id"`
	...
	for _, r := range mp.Results {
		if r.EntryID == entryID {
			t.Errorf("disputed entry %s appeared in match results — Layer 0 gate failed", entryID[:8])
		}
	}
}
