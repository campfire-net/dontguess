...
func TestSettle_AdjustsScripOnComplete(t *testing.T) {
	...
	// Build the antecedent chain required for seller key derivation:
	...
	// Replay all messages including buyer-accept and deliver so the chain is in state.
	...
	// Replay again to include the complete message, then dispatch it.
	allMsgs, _ = h.st.ListMessages(h.cfID, 0)
	eng.State().Replay(allMsgs)
